...
def get_people():
    ...
    # search.map((item)=>{item.serialize()})
...
def post_favorite_people_register():
    ...
    try:
        search2 = FavoritePeople.query.filter_by(
            user_id=id, people_id=people_id).first()
        if search2:
...
def login():
    ...
    if user is None:
        raise APIException(
            "El usuario o el password son incorrectos", status_code=400)

    coincidencia = bcrypt.check_password_hash(
        user.password, password)  # si coincide, devuelve True
...
def add_planets():
    try:
        ...
        return jsonify({"message": "El planeta se agregó"}), 200
    except Exception as error:
        print(error)
        return jsonify({"message": str(error)}), 500
...
def get_favorite_planets_user_id():
    '''
    Esta función va a devolver la lista de planetas favoritos de un usuario en particular por su id
    '''
    body = request.get_json()
    print("body: ", body)
    id = body["id"]
...
def post_favorite_planets_register():
    ...
    try:
        search2 = FavoritePlanets.query.filter_by(
            user_id=id, planets_id=planets_id).first()
        if search2:
            return jsonify({"message": "ya existe ese favorito"}), 409

        new_register = FavoritePlanets(user_id=id, people_id=planets_id)
        db.session.add(new_register)
        db.session.commit()
